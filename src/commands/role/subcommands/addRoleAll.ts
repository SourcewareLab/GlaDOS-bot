...
export async function addRoleAll(interaction: ChatInputCommandInteraction) {
    ...
    if (((role.permissions.valueOf() as bigint) & PermissionsBitField.Flags.Administrator) === PermissionsBitField.Flags.Administrator) {
        await interaction
          .editReply({
    ...
    (await interaction.guild.members.fetch()).values()
      .forEach(async (member) => {
        ...
        if (((member.permissions.valueOf() as bigint) & PermissionsBitField.Flags.Administrator) === PermissionsBitField.Flags.Administrator 
            || member.user.bot 
