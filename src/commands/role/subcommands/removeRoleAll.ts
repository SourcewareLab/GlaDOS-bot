import { ChatInputCommandInteraction, MessageFlags, PermissionsBitField, Role } from "discord.js";



export async function removeRoleAll(interaction: ChatInputCommandInteraction) {
    ...
    const role = interaction.options.getRole("remove_role") as Role
    ...
    try {
        await interaction.editReply({content: `Unassigning role @${role.name} , please wait...`,})
    } catch (err) {
    ...
    (await interaction.guild.members.fetch()).values().forEach(async (member) => {
        // if member is an admin or member is a bot or a member doesnt have the role
        if (
      ...
      await interaction.editReply({
        content: `Successfully removed role @${role.name} from all users.`,
      });
}