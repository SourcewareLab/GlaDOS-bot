...
export async function replaceRoleAll(interaction: ChatInputCommandInteraction) {
    ...
    if (
    (((roleToAdd.permissions.valueOf() as bigint) & PermissionsBitField.Flags.Administrator) === PermissionsBitField.Flags.Administrator) ||
    (((roleToRemove.permissions.valueOf() as bigint) & PermissionsBitField.Flags.Administrator) === PermissionsBitField.Flags.Administrator)
    ) {
    await interaction
        .editReply({
